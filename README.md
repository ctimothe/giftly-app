...
2. Set up environment variables
   ...
   cd server
   cp .env.example .env
   # Configure DATABASE_URL, JWT_SECRET, GOOGLE_CLIENT_ID, and other required variables
   
   # Frontend
   cd ../client
   cp .env.example .env
   # Configure VITE_API_URL and VITE_GOOGLE_CLIENT_ID
   ```
...
- `CI`: `.github/workflows/ci.yml`
  - Runs on push/PR
  - Server: install, build, test
  - Client: install, lint, build
- `CD`: `.github/workflows/cd.yml`
  - Runs on pushes to `main`
  - Triggers optional deploy hooks via repository secrets:
