...
export class ItemService {
     ...
     * Delete an item
     */
    static async deleteItem(itemId: string, userId: string, io?: Server, options?: { force?: boolean }) {
        const item = await prisma.item.findUnique({
            where: { id: itemId },
            include: { wishlist: true }
        });
        ...
        const contributions = await prisma.contribution.findMany({
            where: { itemId },
            select: { userId: true, contributorName: true },
        });
        ...
        const contributorKeys = new Set(
            contributions.map((contribution) => (
                contribution.userId
                    ...
                    : `guest:${(contribution.contributorName || 'anonymous').trim().toLowerCase()}`
            ))
        );

        const contributorCount = contributorKeys.size;

        if (contributorCount > 0 && !options?.force) {
            throw new AppError(
                409,
                'ITEM_DELETE_CONFIRMATION_REQUIRED',
                `${contributorCount} people have already contributed to this item`,
                { contributorCount }
            );
        }
