...
router.delete('/:id', authenticateToken, async (req: Request, res: Response) => {
    try {
        ...
        const result = await ItemService.deleteItem(itemId, req.user!.id, io, { force: forceDelete });

        res.json({ message: 'Item deleted', contributorCount: result.contributorCount });
    } catch (error: any) {
        if (error instanceof AppError) {
