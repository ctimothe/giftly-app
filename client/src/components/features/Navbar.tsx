...
export default function Navbar() {
    ...
    return (
        <nav className="sticky top-0 z-50 border-b border-white/10 bg-slate-950/55 backdrop-blur-xl shadow-sm">
            <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
                {/* Logo — clickable, navigates to dashboard */}
                <Link to="/" className="flex items-center gap-2 group">
                    <div className="h-8 w-8 rounded-lg bg-primary/90 flex items-center justify-center transition-transform group-hover:scale-105 shadow-sm">
                    ...
                    {user ? (
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="ghost" className="flex items-center gap-2 px-2 hover:bg-muted/60 transition-colors cursor-pointer rounded-lg h-auto py-1.5 ring-offset-background outline-none focus-visible:ring-2 focus-visible:ring-ring">
                                    {/* Avatar circle with initials */}
                                    <div className="h-8 w-8 rounded-lg bg-secondary/90 flex items-center justify-center text-xs font-semibold text-secondary-foreground border border-white/10">
                                        {initials}
                                    </div>
                                    <span className="text-sm text-foreground font-medium hidden md:inline-block max-w-[120px] truncate">{user.name || user.email}</span>
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end" className="w-56 bg-popover/95 text-popover-foreground border-white/10 shadow-xl rounded-xl p-1 backdrop-blur-md">
                                {/* User info header */}
            ...
            {mobileOpen && (
                <div className="sm:hidden border-t border-white/10 bg-slate-950/90 px-4 py-4 space-y-2 shadow-lg animate-in slide-in-from-top-2 fade-in duration-200 backdrop-blur-xl">
                    {user ? (
                        <>
                            {/* User info */}
